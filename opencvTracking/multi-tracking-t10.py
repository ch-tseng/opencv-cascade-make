...
FILE_OUTPUT = '/media/sf_VMshare/t10.avi'
frames_tracking = 60
trackerType = "CSRT"
detectDirectionPeriod = 3
...
def getDirection(x_var, y_var, lastDir):
    ...
    elif(abs(x_var)<=2 and abs(y_var)<=2):
        direction = lastDir
...
def imgDirection(img, direction, posXY):
    ...
    result = transparentOverlay(img, dirImg, posXY, 1.0)
    return result
...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture("/media/sf_VMshare/videos/t10.mp4")
    ...
    while True:
        ...
        if r:
            ...
            if(trackingIndex>=frames_tracking):
                ...
                for cat, score, bounds in results:
                    ...
                    x, y, w, h = bounds
                    if(label in ("car", "bus", "motorbike", "truck", "person") ):
        ...
        for id, newbox in enumerate(boxes):
            if(justDetected==True):
                ...
                if(dirCar[id] != "stop_stop"):
                    ...
                    #frame = transparentOverlay(frame, objImg, (int(newbox[0]-20), int(newbox[1])), 1.2)
                    cv2.putText(frame, speed, (int(newbox[0]), int(newbox[1])), cv2.FONT_HERSHEY_COMPLEX, 0.85, fontcolor, fontbold)
                    if(dirCar[id]!=""):
        ...
        cv2.imshow("preview", imutils.resize(frame, width=750))
